...
# This script obtains lfs quota per-user per-filesytem and writes it to node-exporter collector file

# Node exporter textfile collector directory
COLLECTOR="/opt/node-exporter/textfile-collector"

# Create output file
OUTPUT="${COLLECTOR}/user-lustre-storage-quota.$$"
touch "${OUTPUT}"
...
# Populating user_list with users from a dir if USER_DIR is set, else collecting from /etc/passwd
if [[ -z "${USER_DIR}" ]]
then
...
do
	...
	do
		...
		echo "node_user_storage_quota{user=\"${user}\", fs_type=\"${FS_TYPE}\", mount_path=\"${mount_path}\", field=\"kbs_used\"} ${kbs_used}" >> ${OUTPUT}
